...
def _ends_with_polite_declarative(text: str) -> bool:
    """한 줄 요약이 '~합니다체' 평서문으로 끝나는지 검사 (문체 일관성 fail-loud용).

    존댓말 평서문은 항상 '다'로 끝남(입니다/습니다/됩니다/바랍니다…).
    명사형 종결('~주간', '~활동')·개조식('~함', '~했음', '~됨', '~임')은 '다'로 안 끝남.
    빈 문자열은 별도 fallback 경로가 처리하므로 통과시킴.
    """
    if not text:
        return True
    # 끝쪽 공백·인용부호·마침표·이모지성 기호 제거 후 마지막 글자 확인
    stripped = text.strip().rstrip("\"'”’.。！!~ ")
    return stripped.endswith("다")
...
- ⚠️ **문체 통일 (필수)**: 모든 문장은 정중한 '~합니다체'로, "~입니다"·"~습니다"·"~됩니다" 등 완결된 종결어미로 끝낼 것.
  · 명사형 종결 금지: "~논의", "~움직임", "~상황" 처럼 명사로 끝내지 말 것
  · 개조식 종결 금지: "~함", "~했음", "~됨", "~임" 같은 형태 금지
...
2-3문장 설명."""
    ...
    def _generate_master_details(self, stats: Dict[str, Any]) -> str:
        ...
        # 문체 일관성 점검 (fail-loud): 존댓말 평서문은 "~다"로 끝남.
        # 명사형("~주간", "~활동")·개조식("~함", "~했음")으로 새면 경고. (모델 교체 시 drift 감지)
        drifted = [
            name for name, ins in insights.items()
            if not _ends_with_polite_declarative(ins.get("summary", ""))
        ]
        if drifted:
            print(f"  ⚠️ 문체 drift 감지 (summary가 ~합니다체로 안 끝남): {', '.join(drifted)}")
...
## 작성 원칙
- ⚠️ **문체 통일 (필수)**: 모든 문장은 정중한 '~합니다체'로, "~입니다"·"~습니다"·"~됩니다" 등 완결된 종결어미로 끝낼 것. (주차별로 문체가 달라지면 안 되므로 반드시 준수)
  · 명사형 종결 금지: "~커뮤니티 활동", "~확인된 주간", "~논의" 처럼 명사로 끝내지 말 것 → "~커뮤니티 활동이 이어졌습니다", "~확인된 주간입니다"
  · 개조식 종결 금지: "~함", "~했음", "~됨", "~임" 같은 형태 금지
...
- ⚠️ **태그 통계 본문 노출 금지**: "감사 태그 77건", "지정학적 리스크(38건)", "'감사' 80건, '존경' 42건" 처럼 태그명·건수를 본문이나 summary에 쓰지 말 것. 태그는 주제 파악용 참고 자료일 뿐이며, 반드시 내용 자체로 서술할 것.
...
1. **summary**: 한 줄 요약. 데이터에 기반한 팩트 중심으로 **간결한 한 문장**으로 작성 (쉼표로 여러 절을 길게 이어 붙이지 말 것, 통계 수치 나열 금지). **반드시 "~입니다" 또는 "~습니다"로 끝나는 완결된 존댓말 문장** (명사형 "~주간"·"~활동" 또는 개조식 "~함"·"~했음" 종결 금지). (예: "포트폴리오 구성과 종목 관련 질문이 중심인 주간입니다.")

2. **main_content**: 주요 내용을 테마별로 정리 ({('2개 테마' if compact else '2~3개 테마, 최대 3개')})
   - 서비스 피드백/불편사항/제보 관련 내용은 여기에 넣지 말 것. service_feedback에서 별도로 다룸.
   - 비슷한 내용끼리 묶어서 테마로 구성
   - ⚠️ 각 테마는 **핵심만 간결하게 2문장**으로 설명할 것. 배경·맥락을 장황하게 늘어놓거나 같은 말을 부연하지 말 것.
   - 대표 인용문은 가장 핵심적인 원문을 그대로 사용
   - ⚠️ **가장 중요한 주제 2~3개만** 다룰 것. 사소하거나 부차적인 주제까지 빠짐없이 나열하지 말 것 (테마를 억지로 4개 이상으로 늘리지 말 것).
   - 테마 제목은 구체적으로 (예: "이란-미국 전쟁과 예측 신뢰성 논쟁", "커뮤니티 내 갈등 심화")
   반드시 아래 형식의 문자열로 작성해주세요. 각 테마 사이에 반드시 빈 줄(\\n\\n)을 넣어주세요:

**1. 구체적 테마 제목**\\n\\n테마에 대한 간결한 2문장 설명. 핵심 맥락과 이용자 입장만 서술.\\n\\n> _"대표적인 원문 인용"_\\n\\n\\n**2. 구체적 테마 제목**\\n\\n테마에 대한 간결한 2문장 설명.\\n\\n> _"대표적인 원문 인용"_
